...
df["Employment Percent Change, 2021-2031"] = df[
    "Employment Percent Change, 2021-2031"
].astype(float)

df["Onet"] = df["Occupation Code"].str[:2].astype(int)
df["Onet"] = df["Onet"].apply(lambda x: 0 if x is None or x == 0 or np.isnan(x) else x)
...
df = df.rename(
    columns={
        "Typical Entry-Level Education": "Typical_Entry_Level_Education",
        "Typical on-the-job Training": "Typical_on_the_job_Training",
        "Employment Percent Change, 2021-2031": "Employment_Percent_Change_2021_2031",
    }
)
dff = df.copy()
dfg = (
    df.groupby("Onet")[
        [
            "Employment_Percent_Change_2021_2031",
            "Median Annual Wage 2021",
            "Occupational Openings, 2021-2031 Annual Average",
        ]
    ]
...
dfb = (
    df.groupby("Typical_Entry_Level_Education")[
        [
            "Employment_Percent_Change_2021_2031",
            "Median Annual Wage 2021",
            "Occupational Openings, 2021-2031 Annual Average",
...
growth = st.sidebar.radio("Select growth:", ("All", "Negative", "Positive"))
if growth == "All":
    df = df.copy()
...
dfstate = dfstate.rename(
    columns={
        "Occ. \nCode ": "Occupation_Code",
        "Occupational Title ": "Occupation",
        "2030\nEmployment": "2031_Openings",
        "Percent\nChange": "Employment_Percent_Change_2021_2031",
        "Annual Total \nOpenings": "Annual_Openings",
    }
)
# drop the first row
dfstate = dfstate.drop(0)
dfstate["Employment_Percent_Change_2021_2031"] = (
    dfstate["Employment_Percent_Change_2021_2031"] * 100
)
dfstate["Onet"] = dfstate["Occupation_Code"].str[:2].astype(int)
dfstate["Onet"] = dfstate["Onet"].apply(lambda x: 0 if x is None or x == 0 or np.isnan(x) else x)
dfstate = dfstate.replace({"Onet": Onet})
# Calculate the sum of 2031_Openings for all occupations
total_openings = dfstate.loc[
    dfstate["Occupation"] == "Total, All Occupations", "2031_Openings"
].values[0]
...
# drop the first row
df7 = df7.drop(0)
df7["Employment_Percent_Change_2021_2031"] = (
    df7["Employment_Percent_Change_2021_2031"] * 100
)
df7["Onet"] = df7["Occupation_Code"].str[:2].astype(int)
df7["Onet"] = df7["Onet"].apply(lambda x: 0 if x is None or x == 0 or np.isnan(x) else x)
df7 = df7.replace({"Onet": Onet})
# Calculate the sum of 2031_Openings for all occupations
total_openings = df7.loc[
    df7["Occupation"] == "Total, All Occupations", "2031_Openings"
].values[0]
...
dfeduc = dfeduc.melt(
    id_vars=["Month"], var_name="Education", value_name="Unemployment_Rate"
)
dfeduc = dfeduc.sort_values(
    by=["Education", "Unemployment_Rate"], ascending=[True, False]
)
